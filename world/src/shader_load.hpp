...
class CShader {
public:
    static std::shared_ptr<CShader> load_shader_width_path(const char* _vertex_file_path, 
                                                        const char * _pixel_file_path, 
                                                        const char * _geometry_fiel_path);
    static std::shared_ptr<CShader> load_shader(const char* _vertex_shader, const char* _pixel_shader, const char* _geometry_shader);
...
std::shared_ptr<CShader> 
CShader::load_shader_width_path(const char * _vertex_file_path, 
                                const char * _pixel_file_path, 
...
std::shared_ptr<CShader> 
CShader::load_shader(const char* _vertex_shader, const char* _pixel_shader, const char* _geometry_shader) {
    STD_NAMESPACE_DECLARE
    ...
    auto check_compile_error_func = [](SHADER_FILE_TYPE _shader_type, GLuint _shader_id){
        GLint success;
		GLchar infoLog[1024];
        constexpr char* shader_file_type_str[3] = { "vertex shader", "pixel shader", "gemotry shader" };
    ...
    return make_shared<CShader>(program_id);
}
